...
namespace ConsoleAdventure.Content.Scripts.UI
{
    public class Menu
    {
        ...
        public void MenuInit()
        {
            ...
            aboutGamePanel = new InfoPanel(new Rectangle(ConsoleAdventure.screenWidth / 2 - 64, 40 / 2, 64, 30), TextAssets.About, TextAssets.AboutGame);

            aboutControlPanel = new InfoPanel(new Rectangle(ConsoleAdventure.screenWidth / 2 - 80, 40 / 2, 64, 30), TextAssets.Control, TextAssets.AboutControl);
            ...
            for (int i = 0; i < menuSettingsButtons.Length; i++)
            {
                ...
                menuSettingsButtons[i] = new MenuButton(new Vector2((ConsoleAdventure.Width / 2), startPos + (indent * i)), text, new Color(230, 230, 230), menuSettingsButtonTypes[i]);
            }
        ...
        public void MenuUpdate()
        {
            ...
            if (State == 1)
            {
                for (int i = 0; i < worldPanels.Count; i++)
                {
                    ...
                    if (ConsoleAdventure.kstate.IsKeyDown(Keys.Enter) && timer >= Utils.StabilizeTicks(30))
                    {
                        ...
                        {
                            ConsoleAdventure.world.generator.Generate(ConsoleAdventure.world.seed);
                            Saves.Load("World");
            ...
            if (State == 2)
            {
                for (int i = 0; i < menuSettingsButtons.Length; i++)
                {
                    ...
                    {
                        if (menuSettingsButtons[i].type == 0)
                        {
                            ...
                            ConsoleAdventure.world = new World();
                        }
